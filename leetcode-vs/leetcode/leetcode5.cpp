...
public:
	...
	void dfs_allPathsSourceTarget(int cur, vector<int> &path, vector<int> &vis, int target, vector<vector<int>> &grid,
	...
	void dfs_812(int s, int n, vector<int> &path, vector<vector<int>> &points, vector<bool> &vis, double &ans) {
		if (n == 0) {
			int x1 = points[path[0]][0], y1 = points[path[0]][1];
			int x2 = points[path[1]][0], y2 = points[path[1]][1];
			int x3 = points[path[2]][0], y3 = points[path[2]][1];
			ans = max(ans, 0.5 * fabs(x1 * y2 + x2 * y3 + x3 * y1 - x1 * y3 - x2 * y1 - x3 * y2));
			// cout << path[0] << " " << path[1] << " " << path[2] << " " << ans << endl;
			return;
		}
		for (int i = s; i < points.size(); ++i) {
			if (!vis[i]) {
				vis[i] = 1;
				path.push_back(i);
				dfs_812(i + 1, n - 1, path, points, vis, ans);
				path.pop_back();
				vis[i] = 0;
			}
		}
	...
	double largestTriangleArea(vector<vector<int>> &points) {
		...
		vector<int> path;
		double ans = 0;
		dfs_812(0, 3, path, points, vis, ans);
		return ans;
	}
	...
	double largestSumOfAverages(vector<int> &A, int K) {
		auto len = A.size();
		vector<vector<double>> dp(len, vector<double>(K + 1));
		...
		for (auto &it : st) {
			...
			for (int i = 0; i < buses.size(); ++i) {
				for (int j = i + 1; j < buses.size(); ++j) {
					graph[buses[i]].insert(buses[j]);
					graph[buses[j]].insert(buses[i]);
				}
			}
		}
		int ans = 1;
		unordered_set<int> vis, dest;
		for (auto bi : st[T]) {
			dest.insert(bi);
		}
		deque<int> q;
